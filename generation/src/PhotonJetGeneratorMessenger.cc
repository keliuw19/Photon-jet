...
PhotonJetGeneratorMessenger::PhotonJetGeneratorMessenger(PhotonJetPrimaryGenerator* primaryGenerator)
	: fPrimaryGenerator(primaryGenerator)
{
	generatorDirectory = new G4UIdirectory("/PhotonJet/generator/");
	generatorDirectory->SetGuidance("Primary generator initialization parameters.");

	cmd_gen_minPrimaryMomentum = new G4UIcmdWithADoubleAndUnit("/PhotonJet/generator/minMomentum", this);
	...
	cmd_gen_maxPrimaryMomentum = new G4UIcmdWithADoubleAndUnit("/PhotonJet/generator/maxMomentum", this);
	...
	cmd_gen_sourcePosition = new G4UIcmdWith3VectorAndUnit("/PhotonJet/generator/sourcePosition", this);
	...
	cmd_gen_particle = new G4UIcmdWithAString("/PhotonJet/generator/particleName", this);
	...
	cmd_gen_dump = new G4UIcmdWithoutParameter("/PhotonJet/generator/dump", this);

	fPrimaryGenerator->setMinPrimaryMomentum( PhotonJetPrimaryGenerator::default_minPrimaryMomentum );
	fPrimaryGenerator->setMaxPrimaryMomentum( PhotonJetPrimaryGenerator::default_maxPrimaryMomentum );
	fPrimaryGenerator->setSourcePosition( G4ThreeVector(0.0, 0.0, -PhotonJetPrimaryGenerator::default_sourceDistance) );
	fPrimaryGenerator->setParticleName( PhotonJetPrimaryGenerator::default_particleName );
}

PhotonJetGeneratorMessenger::~PhotonJetGeneratorMessenger()
{
...
void PhotonJetGeneratorMessenger::SetNewValue(G4UIcommand* command, G4String newValues)
{
...
G4String PhotonJetGeneratorMessenger::GetCurrentValue(G4UIcommand* command)
{
	G4String cv;
