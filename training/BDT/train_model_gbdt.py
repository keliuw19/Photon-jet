...
def main():
    ...
    tree3 = fin3.Get("fancy_tree")
    backgr3 = tree3.AsMatrix(columns=branch_names)
    signal2 = backgr3[:20000,:]
...
#    plot_learning_curve("./bdt_results/", bdt, X_train, y_train)
    ...
    plot_inputs("./bdt_results_5para/", branch_names, signal, None, backgr, None,signal2,None)
...
def plot_inputs(outdir, vars, sig, sig_w, bkg, bkg_w, sig2, sig2_w):
    for n, var in enumerate(vars):
        _, bins = np.histogram(np.concatenate((sig[:, n], bkg[:, n],sig2[:, n])), bins=40)
        ...
        sns.distplot(sig2[:, n], hist_kws={'weights': sig2_w}, bins=bins, kde=False, norm_hist=True, label='signal2')
        plt.title(var)
        plt.legend()
